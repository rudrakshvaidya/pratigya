window.onload = () => {
  ...
  finalMessage.innerHTML = "May this day be as bright and beautiful as your smile. Wishing you all the happiness in the world. <br>I love you, Pratigya.";
  ...
  function createIntroAnimation() {
    ...
    tl.to(lotusScene, { opacity: 1, duration: 2.5, ease: 'power2.inOut' });
  ...
  tl.to(lotusScene, { opacity: 0, duration: 1.5 })
    .set(lotusScene, { display: 'none' });

  // Step 2: SIMULTANEOUSLY darken background, show the scene, and animate in the cake/flame
  tl.set(cakeScene, { display: 'flex', opacity: 1, pointerEvents: 'auto' }, "<")
    .to(body, { backgroundColor: '#1a1a1a', duration: 1.5 }, "<")
    .from(cakeContainer, { y: 100, opacity: 0, duration: 1.5, ease: 'power2.out' }, "<")
...
function handleFlameClick() {
  ...
  celebrateTl.to(body, { backgroundColor: '#fff', duration: 1 })
    .to(cakeContainer, { y: '120vh', ease: 'power2.in', duration: 1.2 }, '<')
    .set(cakeScene, { display: 'none' });
    
  celebrateTl.set(celebrationScene, { display: 'flex', opacity: 1, pointerEvents: 'none' })
    .from(birthdayText, { scale: 0.3, opacity: 0, ease: 'back.out(1.7)', duration: 0.8 })
    .call(() => {
      // Start DENSE continuous celebration
      celebrationInterval = setInterval(() => {
        for(let i=0; i<3; i++) createEffect('confetti-piece', 5 + Math.random() * 5);
        if (Math.random() > 0.8) createEffect('balloon', 9 + Math.random() * 7);
      }, 70);
    });
  ...
  mainTimeline.add(createIntroAnimation())
              .add(createCakeTransitionAnimation());
